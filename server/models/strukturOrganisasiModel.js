...
class StrukturOrganisasi {
    static getKetua(callback) {
        const query = 'SELECT foto_div_kurl AS foto, nama_div_kurl AS nama, "Ketua" AS peran FROM divisi_kurl LIMIT 1';
    ...
    static getAnggotaDivisi(callback) {
        ...
        .then(results => callback(null, [].concat(...results)))
        .catch(callback);
    }

    static getSekretaris(callback) {
        const query = 'SELECT foto_sekretaris AS foto, nama_sekretaris AS nama, "Sekretaris" AS peran FROM sekretaris LIMIT 1';
        db.query(query, callback);
    }
